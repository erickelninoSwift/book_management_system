...
const Alert = ({ itemTodeleteID }) => {
  const { alertDelete, setAlertDelete, setAllusersRegisered, cookies } =
    useContext(UserContext);

  const handleDelete = () => {
    axios
      .delete(`http://localhost:8080/contacts/${itemTodeleteID}`, {
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          setAllusersRegisered(response.data.contacts);
